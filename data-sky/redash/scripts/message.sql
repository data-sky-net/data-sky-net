\set options '{"parameters": [{"value": "':message'", "type": "text", "name": "message", "locals": [], "title": "message"}]}'
...
FROM
  (SELECT ins.id AS new_id,
   ...
   JOIN
     (SELECT *
      FROM queries
      WHERE 'for_message'=ANY(tags)) qu ON ins.updated_at=qu.updated_at
   AND ins.created_at=qu.created_at
   WHERE ins.tags::text[]=array[:'message_with_time']) q
JOIN visualizations v ON q.example_id=v.query_id
ORDER BY q.example_id;

INSERT INTO dashboards (updated_at, created_at, VERSION, org_id, slug, name, user_id, layout, dashboard_filters_enabled, is_archived, is_draft)
SELECT now(),
       now(),
       1,
       org_id,
      ...
      FROM
        (SELECT *
         ...
         WHERE query_id IN
             (SELECT id
              FROM queries
              WHERE tags::text[]=array[:'message_with_time'])) v
      JOIN
        (SELECT *
         FROM visualizations
         WHERE query_id IN
             (SELECT id
              FROM queries
              WHERE 'for_message'=ANY(tags))) vi ON vi.updated_at=v.updated_at
